 ...
 */
public class PurchaseRowLayoutTwoAdapter extends BaseAdapter {
    Context context;
    List<?> items;
    ArrayList<Purchases> purchaseList;

    public PurchaseRowLayoutTwoAdapter(Context context,List<?> items){
        this.context=context;
        this.items=items;
        purchaseList=(ArrayList)items;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        name.setText(purchaseList.get(position).getName());
        description.setText("Ksh." + purchaseList.get(position).getAmount());
        description.setTextColor(Color.parseColor("#c27f82"));
        image.setImageDrawable(getDrawable(purchaseList.get(position).getName().substring(0,1).toUpperCase()));
        date.setText(purchaseList.get(position).getDate());
        time.setText(purchaseList.get(position).getTime());
