...
import com.myaccountant.myaccountant.adapters.OtherIncomeRowLayoutOneAdapter;
 ...
 */
public class OtherIncomeListFragment extends Fragment {
    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView=inflater.inflate(R.layout.other_income_list_layout,container,false);
        ListView listView=(ListView)rootView.findViewById(R.id.other_income_list);
        RelativeLayout parent=(RelativeLayout)rootView.findViewById(R.id.other_income_list_parent);
        SnackBar snackBar=SnackBar.make(getActivity());

        DatabaseHelper helper=new DatabaseHelper(getActivity());
        List<OtherIncome> items=helper.getAll(OtherIncome.class);

        listView.setAdapter(new OtherIncomeRowLayoutOneAdapter(getActivity(),items));
    ...
    public double getTotal(ArrayList<OtherIncome> items){
        double total=0;
        for(OtherIncome list:items){
